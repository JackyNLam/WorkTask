...
class TaskWindow:
    ...
    def pause_working(self):
        ...
        con.commit()
        del self.start_time,self.working_item
        self.refresh_task_list("Pending")
...
class NoteWindow:
    def __init__(self, master):
        ...
        if len(self.Address_Info)>0:
            ...
            self.Address_Info = ""
        self.Address_Entry_Box = Text(self.AddressFrame,width=50)
        self.Address_Entry_Box.insert(1.0,self.Address_Info)
        self.Address_Entry_Box.grid(row=2,column=1,padx=20,ipady=100)
